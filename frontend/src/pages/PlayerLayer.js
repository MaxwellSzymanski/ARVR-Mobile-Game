...
class PlayerLayer extends React.Component {
    ...
    sendLocation() {
        ...
        if (history["you"] === undefined) {
            let arr = [];
            arr.push(myData);
            history["you"] = arr;
        } else {
            history["you"].push(myData);
        }
        if (history["you"].length >= 15) {
            history["you"].shift();
        }
        this.setState({historyDataPlayers: JSON.stringify(history)});

        myData.token = cookies.get('token');
        this.context.emit("location", myData);
    }
    ...
    receivePlayer(data) {
        ...
        list[data.id].latitude = 0.7*data.latitude + 0.3*list[data.id].latitude;
        list[data.id].longitude = 0.7*data.longitude + 0.3*list[data.id].longitude;
        list[data.id].updatedAt = data.updatedAt;
        list[data.id].accuracy = data.accuracy;
        this.setState({dataPlayers: list});

        let history = JSON.parse(this.state.historyDataPlayers);
        if (history[data.id] === undefined) {
            let arr = [];
            arr.push(list[data.id]);
            history[data.id] = arr;
        } else {
            history[data.id].push(list[data.id]);
        }
    ...
    showFindEnemyAlertBox(){
        ...
        rows.push(<button className="confirmButton wider" onClick={playerLayer.props.setTargetMySelf.bind(playerLayer,[this.state.latitude,this.state.longitude])}>Follow yourself</button> );
        ...
        if(jsonObject !== null){
            ...
            Object.keys(jsonObject).forEach(function(key) {
                ...
                if(idEnemy !== playerLayer.state.id){
                    rows.push(<button className="confirmButton wider" onClick={playerLayer.props.setTarget.bind(playerLayer,idEnemy,[playerData.latitude,playerData.longitude])}>Track {idEnemy}</button>);
                 }
            });
        }
